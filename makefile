...
PID_FILE = /tmp/cottrellio-web.pid
...
start:
	go run ./cmd/cottrellio-web/main.go & echo $$! > $(PID_FILE)
# You can also use go build command for start task
# start:
#   go build -o /bin/cottrellio-web . && \
#   /bin/cottrellio-web & echo $$! > $(PID_FILE)
